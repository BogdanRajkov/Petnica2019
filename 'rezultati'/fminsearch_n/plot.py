...
fit = {3:3, 4:4, 5:5, 7:7, 8:8, 9:8, 10:8, 11:9}
...
for i in [3, 4, 5, 7, 8, 9, 10, 11]:
    ...
    if fit[i] != i:
        ...
        plt.scatter(x, y)
        plt.suptitle('Logaritam indeksa prelamanja od udaljenosti')
        plt.xlabel('Udaljenost od centra sfere')
        plt.ylabel('Logaritam indeksa prelamanja')
        plt.savefig('slike/n'+str(i)+'_t.jpg')
        plt.close()
        f.write('m_t = '+str(m_t)+'\n')
        f.write('c_t = '+str(c_t)+'\n')
